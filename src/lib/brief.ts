...
export type { InterimHandoff, FinalHandoff } from './handoff-types.ts';
import type { InterimHandoff, FinalHandoff } from './handoff-types.ts';
...
function renderPattern(spec: { id: string; desc: string; produces: readonly string[]; consumes: readonly (string | { artifact: string })[]; mode?: 'execute' | 'plan'; validate?: readonly any[] }, dagDesc: string, position: string, termContext?: import('../runtime/context.ts').Context): string {
  if (spec.mode === 'plan') {
    return [
      `TASK: ${spec.desc}`,
