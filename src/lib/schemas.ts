...
const nodeSpecInput = z.object({
  ...
  consumes: z.array(z.string()).default([]).describe('File paths this node reads · ordering derives EXCLUSIVELY from consumes ↔ produces (no deps field)'),
...
export const schemas: Record<string, CommandSchema> = {
  ...
  'dag.insert': {
    ...
    examples: [{
      ...
      cli: 'roadmap dag insert --node \'{"id":"add-auth","desc":"Add authentication module","produces":["src/auth.ts"],"consumes":["package.json"],"validate":[{"type":"artifact-exists"}]}\' --note "add auth node"',
    }],
  },
